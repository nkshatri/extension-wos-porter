---
description: "End-to-end ARM64 performance optimization agent: given an open-source app name with a git URL or local source path, it clones/validates the source, delegates an ARM64 build (exe + pdbs) to wos-builder, discovers or generates a benchmark/test suite, captures a CPU-sampling ETL trace against that workload, validates PDB/ETL alignment, then runs hotspot_analysis.py to identify top CPU-hottest functions, applies the full range of Windows ARM64 optimizations (NEON/SVE/SVE2/SME vectorization, scalar/micro-arch tuning, branch/memory/cache improvements, and compiler-flag changes) to those hotspots and their transitive callees, delegates a post-optimization build back to wos-builder and validation to wos-tester, and writes a detailed self-contained HTML report. Never commits anything."
name: "wos-etl-hotspot"
tools: [execute, read, search, edit, todo]
agents: [wos-builder, wos-tester]
user-invocable: true
argument-hint: "Required: open-source app name and git URL (e.g. https://github.com/sqlite/sqlite.git) or local source path"
---
...
1. **Acquire source, build for ARM64, and collect a real ETL trace** — clone or validate the app source, delegate an ARM64 build (`.exe` + `.pdb`) to `wos-builder`, discover or generate a benchmark/test suite, capture a CPU-sampling ETL trace while running that workload, and confirm PDB/ETL alignment.
...
6. **Delegate the post-optimization build** to the `wos-builder` sub-agent to compile the optimized source for ARM64.
...
1. **Application name and source** — either:
   - A **git URL** to clone (e.g. `https://github.com/sqlite/sqlite.git`), or
   - A **local source path** — absolute path to an existing checkout (e.g. `C:\src\sqlite`).

That is the only input needed upfront. All other artifacts (`.exe`, `.pdb`, `.etl`) are produced automatically in Step 1.

Wait for the user to supply this before continuing. Never guess or fabricate paths.
...
- **Never commit anything.** Do NOT run `git commit`, `git add`, `git push`, or any git staging command at any point — not during source acquisition, not after optimizations, not after build/test. Leave all version-control decisions entirely to the user.
...
### Step 1: Acquire Source, Build for ARM64, Collect ETL Trace, and Validate Alignment

This step produces all four inputs required by Step 2 (`<source_dir>`, `<exe_path>`, `<modules_dir>`, `<etl_path>`) from a single user-supplied starting point: an app name plus a git URL or local path.

#### 1a. Acquire the Application Source

The user provides either a **git URL** or a **local source path**:

```powershell
# If a git URL was provided:
git clone <git_url> <source_dir>

# If a local path was provided, validate it exists:
if (-not (Test-Path "<local_path>")) {
    Write-Error "ERROR: Source directory not found: <local_path>"
    exit 1
}
$source_dir = "<local_path>"
```

Record `<source_dir>` as the root of the application source tree for all subsequent steps. If cloning fails (network error, auth required, etc.), stop and report the error verbatim.

#### 1b. Build for Windows ARM64 (delegate to `wos-builder`)

Invoke the **`wos-builder`** sub-agent (from the same `<agents_dir>` as this agent) with `<source_dir>` and instruct it to:

- Build the application for **Windows ARM64** with full debug symbols (`/Zi`).
- Produce **both** the `.exe` (or `.dll`) **and** matching `.pdb` files in the same output folder.
- Return the **absolute paths** of:
  - `<exe_path>` — the ARM64 executable
  - `<modules_dir>` — the folder containing both the `.exe` and `.pdb` file(s)

> `hotspot_analysis.py` scans `<modules_dir>` for all `.exe`, `.dll`, and `.sys` files — both the binary and symbols must be co-located there.

If `wos-builder` reports build errors it cannot self-resolve, surface them verbatim and stop. Do **not** proceed until a successful ARM64 build with PDBs is confirmed.

#### 1c. Discover or Generate a Benchmark / Test Suite

Search `<source_dir>` for an existing benchmark or performance test suite:

```powershell
Get-ChildItem -Path "<source_dir>" -Recurse -Include "*.c","*.cpp","*.py","*.sh","*.bat","*.ps1","CMakeLists.txt","Makefile*" |
    Select-String -Pattern "bench|perf|speed|timer|throughput" -SimpleMatch |
    Select-Object -ExpandProperty Path | Sort-Object -Unique
```

- If a **benchmark harness or speed test is found** (e.g. `speedtest1.c` for SQLite, a `bench/` directory, `*_bench.*`, `*_perf.*`):
  - Build it if it is not already built as part of Step 1b.
  - Use it as `<workload_exe>` for trace collection.
- If **no benchmark exists**:
  - Generate a self-contained stress / exercise driver that exercises the application's key public APIs and hot paths.
  - Write it to `<source_dir>\arm64_bench\bench_driver.c` (or `.cpp`), build it as part of the ARM64 build (ask `wos-builder` to include it), and use the resulting binary as `<workload_exe>`.
  - Document what the generated driver exercises in the HTML report.

Record `<workload_cmd>` — the full command (exe + arguments) to invoke for trace collection.

#### 1d. Collect the ETL Trace

WPR and xperf both require **kernel-mode CPU sampling**, which demands administrative privileges (`SeSystemProfilePrivilege`). Always launch the tracer in an elevated process — never attempt an unelevated trace and then fall back, because a non-elevated WPR/xperf invocation will silently produce an empty or missing ETL.

**Step 1 — Detect elevation and re-launch elevated if needed:**

```powershell
# Check whether the current process is already elevated
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()
            ).IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)

if (-not $isAdmin) {
    Write-Host "Not elevated. Re-launching trace script as Administrator..."
    # Write the trace commands to a temporary script and invoke it elevated
    $traceScript = "$env:TEMP\lame_trace_elevated.ps1"
    # (populate $traceScript with Steps 2-4 below, then:)
    Start-Process powershell.exe `
        -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$traceScript`"" `
        -Verb RunAs `
        -Wait
    # After the elevated process exits, the ETL file will be at $etlPath
} else {
    Write-Host "Running elevated — proceeding with trace capture."
}
```

**Step 2 — Write the elevated trace script to disk:**

Generate `$env:TEMP\lame_trace_elevated.ps1` with the following content (substituting the real paths):

```powershell
# --- lame_trace_elevated.ps1 (runs as Administrator) ---
$etlPath    = "<etl_output_path>"   # e.g. C:\OpenSource\...\lame_arm64_bench.etl
$workloadCmd = "<workload_exe>"      # e.g. lame.exe with args
$wptRoot    = "C:\Program Files (x86)\Windows Kits\10\Windows Performance Toolkit"

# Prefer wpr; fall back to xperf if wpr is absent
$wpr = Get-Command "wpr.exe" -ErrorAction SilentlyContinue
if (-not $wpr) { $wpr = Get-Item "$wptRoot\wpr.exe" -ErrorAction SilentlyContinue }
$xperf = Get-Command "xperf.exe" -ErrorAction SilentlyContinue
if (-not $xperf) { $xperf = Get-Item "$wptRoot\xperf.exe" -ErrorAction SilentlyContinue }

if ($wpr) {
    Write-Host "Starting WPR CPU trace (elevated)..."
    & $wpr.Source -start CPU -filemode
    if ($LASTEXITCODE -ne 0) { Write-Error "WPR start failed: $LASTEXITCODE"; exit 1 }

    Write-Host "Running workload..."
    Invoke-Expression $workloadCmd

    Write-Host "Stopping WPR and saving ETL to $etlPath ..."
    & $wpr.Source -stop $etlPath
    if ($LASTEXITCODE -ne 0) { Write-Error "WPR stop failed: $LASTEXITCODE"; exit 1 }

} elseif ($xperf) {
    Write-Host "WPR not found — using xperf (elevated)..."
    & $xperf.Source -on PROFILE -f $etlPath
    if ($LASTEXITCODE -ne 0) { Write-Error "xperf start failed: $LASTEXITCODE"; exit 1 }

    Write-Host "Running workload..."
    Invoke-Expression $workloadCmd

    Write-Host "Stopping xperf..."
    & $xperf.Source -stop
    if ($LASTEXITCODE -ne 0) { Write-Error "xperf stop failed: $LASTEXITCODE"; exit 1 }

} else {
    Write-Error "Neither wpr.exe nor xperf.exe found. Install the Windows ADK (Windows Performance Toolkit feature)."
    exit 1
}

Write-Host "ETL saved: $etlPath  (size: $([math]::Round((Get-Item $etlPath).Length/1KB)) KB)"
```

**Step 3 — Execute and collect the result path:**

After the elevated script exits (via `Start-Process -Wait`), read `$etlPath` back into the parent session:

```powershell
if (-not (Test-Path $etlPath)) {
    Write-Error "ETL file not found at $etlPath after elevated capture. Check the elevated script output."
    exit 1
}
Write-Host "ETL captured at: $etlPath  ($([math]::Round((Get-Item $etlPath).Length/1KB)) KB)"
```

Rules:
- **Always use the elevated path** (Steps 1–3 above). Never attempt an unelevated WPR/xperf call.
- The workload **must use the ARM64 `.exe` built in Step 1b** — not any pre-existing or x64 binary.
- Target **at least 10–30 seconds** of active CPU work to ensure sufficient samples.
- If the resulting `.etl` is **< 100 KB**, warn: *"Trace may be too short — it may not contain sufficient CPU samples."* Ask the user whether to re-run with a longer workload before continuing.
- If `Start-Process -Verb RunAs` is blocked by a Group Policy or UAC setting, stop and instruct the user to run the parent shell as Administrator before invoking this agent.

Record `<etl_path>` — absolute path to the captured `.etl` file.

#### 1e. Validate PDB / ETL Alignment

Confirm the `.pdb` files in `<modules_dir>` match the exact `.exe` binary that generated the ETL trace — they **must come from the same build invocation**.

```powershell
# Inspect the PE debug directory to read the embedded PDB GUID and age
dumpbin /HEADERS "<exe_path>" | Select-String -Pattern "PDB file|age|GUID" | Write-Host
```

- If the `.exe` and `.pdb` were produced together in Step 1b and the trace was collected immediately after with that same binary, alignment is guaranteed — record this explicitly.
- If there is any suspicion that the binary was rebuilt after trace collection (different timestamps, a second build pass, etc.), **the PDB GUID will not match the ETL symbols**. In that case: delete the stale ETL, repeat Steps 1b–1d with a fresh build, and re-validate.
- **Never proceed to Step 2 with a mismatched PDB.** Symbol resolution will silently fail, producing meaningless or empty hotspot data.

After Step 1e, all four inputs for Step 2 are confirmed:

| Input | Produced in |
|-------|-------------|
| `<source_dir>` | Step 1a |
| `<exe_path>` | Step 1b (`wos-builder`) |
| `<modules_dir>` | Step 1b (`wos-builder`) |
| `<etl_path>` | Step 1d |

Locate `hotspot_analysis.py` before proceeding to Step 2 — check these candidate paths in order and use the first that exists:
...
If `hotspot_analysis.py` cannot be found, stop and report the two candidate paths that were checked. Proceed to **Step 2** only when all inputs and the tool script are confirmed.
...
| Condition | Action |
|-----------|--------|
| Git clone fails (network error, auth, etc.) | Stop and report the full error. Ask the user to provide a local path or fix access. |
| Local source path does not exist | Stop: "Source directory not found: `<path>`." Ask the user to re-supply. |
| `wos-builder` fails ARM64 build (Step 1b) | Surface errors verbatim. Do NOT proceed until a clean ARM64 build with PDBs is confirmed. |
| No benchmark found and generated driver fails to build | Report which build errors the generated driver hit. Ask the user if they want to supply a workload command manually. |
| `wpr` and `xperf` both absent | Stop: "WPR/xperf not found. Install the Windows Performance Toolkit (Windows ADK) to capture ETL traces." |
| WPR/xperf fail with `0xc5585011` / `ERROR_ACCESS_DENIED` / "Failed to enable policy" | **Do not retry unelevated.** These errors mean the tracer is not running as Administrator. Re-launch via `Start-Process powershell -Verb RunAs` (Step 1d) and retry. If UAC or Group Policy blocks elevation, stop and ask the user to re-run their shell as Administrator. |
| `Start-Process -Verb RunAs` is blocked (UAC disabled/Group Policy) | Stop and tell the user: "Please re-launch your terminal or IDE as Administrator, then re-run the agent. WPR/xperf require elevated privileges for kernel CPU sampling." |
| ETL file < 100 KB after collection | Warn: "Trace may be too short — it may not contain sufficient CPU samples. Re-run with a longer workload?" Wait for user confirmation before proceeding to Step 2. |
| PDB GUID does not match the traced `.exe` | Delete the stale ETL and re-run Steps 1b–1d with a fresh build. Never proceed with a mismatched PDB. |
...
| `wos-builder` reports unresolved build errors (Step 8) | Surface the remaining errors verbatim. Do NOT proceed to `wos-tester`. Flag that an applied optimization may have broken the build. |
...
| Agent attempts any `git commit`, `git add`, or `git push` | **Hard stop.** This is forbidden at all times. Leave all version-control decisions to the user. |

## Success Criteria

- **Source acquired**: `<source_dir>` exists and contains the application source (cloned or validated from a local path).
- **ARM64 build confirmed**: `wos-builder` produced an ARM64 `.exe` and matching `.pdb` files in `<modules_dir>`.
- **Workload exists**: an existing benchmark was discovered or a driver was generated and built.
- **ETL captured**: `<etl_path>` points to an ETL > 100 KB collected by running the workload with the ARM64 binary.
- **PDB/ETL aligned**: the `.pdb` in `<modules_dir>` matches the `.exe` used during trace collection (same build invocation).
...
- **No git commits were made at any point** — the working tree is left for the user to review and commit.
- **The `wos-builder` sub-agent is invoked** (from the same `<agents_dir>`) with `<source_dir>` to build the optimized project for ARM64, and its build + dumpbin output is surfaced.
